...
| Действие | Описание |
|----------|----------|
| Создание менеджеров | Регистрирует менеджеров, назначает права делегирования |
| Создание собственников | Регистрирует учётную запись собственника |
| Управление объектами | CRUD объектов, привязка к собственнику |
| Назначение объектов | Привязывает объекты к менеджерам (ManagerProperty) |
| Заезды, операции, выплаты | Те же действия, что у менеджера: заезды, произвольные операции, выплаты — по всем объектам |
| Карточка собственника | Дашборд и вкладки (объекты, заезды, операции, выплаты) — как у менеджера |
| Полный доступ | Видит и может всё в системе — суперпользователь |

### 2.2 Менеджер (MANAGER)

| Действие | Описание |
|----------|----------|
| Создание собственников | Только если делегировано (`canCreateOwners`) |
| Создание объектов | Только если делегировано (`canCreateProperties`) |
| Создание заездов | Только для назначенных объектов |
| Добавление операций | Только для назначенных объектов |
| Создание выплат | Только для собственников с назначенными объектами |
| Просмотр дашбордов | Только по назначенным объектам и связанным собственникам |

**Скоупинг:** менеджер видит только данные по объектам, назначенным ему администратором.

### 2.3 Собственник (OWNER)
...
- Собственник видит **только свои** данные
- Менеджер видит только данные по **назначенным** объектам
...
| role | Enum: ADMIN / MANAGER / OWNER | Роль в системе |
| canCreateOwners | Boolean | Делегированное право создавать собственников (MANAGER) |
| canCreateProperties | Boolean | Делегированное право создавать объекты (MANAGER) |
...
**Связи:** Owner (N:1), Stay (1:N), ManagerProperty (N:M с User), Transaction (1:N), Payout (1:N)

### 3.3.1 ManagerProperty (Назначение менеджера на объект)

| Поле | Тип | Описание |
|------|-----|----------|
| id | UUID | Уникальный идентификатор |
| userId | UUID | ID менеджера |
| propertyId | UUID | ID объекта |
| assignedAt | DateTime | Дата назначения |

**Связи:** User (N:1), Property (N:1). Уникальная пара (userId, propertyId).
...
| propertyId | UUID | Связь с объектом (обязательно) |
...
| id | UUID | Уникальный идентификатор |
| ownerId | UUID | Связь с Owner |
| propertyId | UUID | Связь с Property (обязательно) |
...
| Метод | Путь | Доступ | Описание |
|-------|------|--------|----------|
| POST | `/api/users/owner` | Admin | Создать собственника |
| POST | `/api/users/manager` | Admin | Создать менеджера |
| GET | `/api/users/managers` | Admin | Список менеджеров |
| DELETE | `/api/users/:id` | Admin | Удалить менеджера |

### 5.2.1 Назначения менеджеров (Manager Properties)

| Метод | Путь | Доступ | Описание |
|-------|------|--------|----------|
| POST | `/api/manager-properties` | Admin | Назначить объект на менеджера |
| DELETE | `/api/manager-properties/:id` | Admin | Снять назначение |
| GET | `/api/manager-properties` | Admin | Список назначений (фильтр userId, propertyId) |

### 5.3 Собственники

| Метод | Путь | Доступ | Описание |
|-------|------|--------|----------|
| GET | `/api/owners` | Admin / Manager | Список собственников (Manager — только доступные) |
...
| Метод | Путь | Доступ | Описание |
|-------|------|--------|----------|
| POST | `/api/properties` | Admin | Создать объект |
| GET | `/api/properties` | Admin / Manager / Owner | Список (Manager — назначенные, Owner — свои) |
| GET | `/api/properties/:id` | Admin / Manager / Owner | Карточка объекта |
| PATCH | `/api/properties/:id` | Admin | Обновить объект |
| DELETE | `/api/properties/:id` | Admin | Удалить объект |

### 5.5 Заезды

| Метод | Путь | Доступ | Описание |
|-------|------|--------|----------|
| POST | `/api/stays` | Admin / Manager | Создать заезд (Manager — только назначенные объекты) |
...
| Метод | Путь | Доступ | Описание |
|-------|------|--------|----------|
| POST | `/api/transactions` | Admin / Manager | Создать операцию (Manager — только назначенные объекты) |
...
| Метод | Путь | Доступ | Описание |
|-------|------|--------|----------|
| POST | `/api/payouts` | Admin / Manager | Создать выплату (Manager — только по своим объектам) |
...
| GET | `/api/dashboard/owner/:id` | Admin / Manager | Дашборд собственника (Manager — если есть назначенные объекты) |
...
- Декоратор `@Roles(Role.ADMIN)` / `@Roles(Role.MANAGER)` — ограничение по ролям
- ADMIN — суперпользователь, проходит любую проверку ролей
- Декоратор `@Public()` — открытый эндпоинт (логин, refresh)

### 6.3 Создание учётных записей

- Администратор создаёт менеджеров и собственников
- Администратор может делегировать менеджеру права `canCreateOwners`, `canCreateProperties`
- Собственник **не может** зарегистрироваться самостоятельно
- Первый администратор создаётся через seed-скрипт (`admin@balivi.ru`)
...
### 7.3 Кабинет администратора

- Менеджеры — создание, список, удаление
- Собственники — создание, список; клик по строке — карточка собственника (дашборд и вкладки, как у менеджера)
- Объекты — создание, список, CRUD
- Назначения — привязка объектов к менеджерам
- Заезды — список заездов, создание заезда (`/admin/bookings`, `/admin/bookings/create`) — те же экраны, что у менеджера
- Операции — таблица операций с фильтрами и добавлением (`/admin/transactions`)
- Выплаты — таблица выплат и создание (`/admin/payouts`)

Маршруты зеркалируют кабинет менеджера под префиксом `/admin/...`, кроме разделов, доступных только администратору (менеджеры, назначения).

### 7.4 Кабинет менеджера
...
│   │   │   └── dto/               # LoginDto
│   │   ├── users/                 # createOwner, createManager, getManagers, deleteManager
│   │   ├── manager-properties/    # назначения объектов на менеджеров
│   │   ├── owners/
│   │   ├── properties/
│   │   ├── stays/
│   │   ├── transactions/
...
│   │   │   ├── admin-layout/       # Сайдбар + шапка администратора
...
- Администратора по умолчанию (email: `admin@balivi.ru`, пароль: `admin123`)
