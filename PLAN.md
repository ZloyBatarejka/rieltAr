...
- [x] **1.3** Создать React frontend
  ...
  - Установить зависимости: Chakra UI, axios, react-router-dom
  - Настроить базовую структуру папок (FSD)

- [x] **1.4** Настроить Chakra UI
  - Подключить `ChakraProvider` с темой и semanticTokens
...
**Результат:** backend на :8070 отдаёт `GET /api/health → { status: "ok" }`, frontend на :3070 показывает заглушку с Chakra UI.
...
- [x] **2.8** Авторизация UI
  ...
  - Страница логина с формой (Chakra UI + RHF + Zod)
...
**Таблицы:** TanStack Table (`@tanstack/react-table`) + Chakra UI Table — для всех списков (собственники, объекты, назначения, операции, выплаты). См. `.cursor/rules/frontend.mdc`.
